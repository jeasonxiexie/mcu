/**
 * @file lcd_v1_no_offset.c
 * @brief LCD driver variant 1 - No X-axis offset (standard mode)
 * @version v1
 */
...
void LCD_GPIO_Init(void)
{
    ...
    /* Enable clocks for both GPIOB (backlight) and GPIOC (LCD interface) */
    __HAL_RCC_GPIOB_CLK_ENABLE();
    __HAL_RCC_GPIOC_CLK_ENABLE();
    
    /* LCD interface pins on GPIOC */
...
void LCD_SetAddress(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2)
{
    ...
    LCD_WriteData8(0x00);
    LCD_WriteData8(x1);  /* No offset */
    LCD_WriteData8(0x00);
    LCD_WriteData8(x2);  /* No offset */
