...
int main(void)
{
  ...
  Tim1_Config();  /* 保持PC6的TIM1 PWM配置不变！ */
  
  Power_LED_Config();  /* 新增：电源和LED配置 */

  /* PC4 as TIM10 Togggle Single - 改为PC0 */
  /* RCC_AHBPeriphClockCmd(RCC_AHBPeriph_GPIOC, ENABLE); 已经在Power_LED_Config中使能 */
  /* GPIO_Init(GPIOC, GPIO_Pin_4, GPIO_MODE_OUT | GPIO_PUPD_NOPULL | GPIO_OTYPE_PP | GPIO_SPEED_HIGH); */
  
  /* PD4 as TIM10 EXT Single - 保持不变，不影响 */
