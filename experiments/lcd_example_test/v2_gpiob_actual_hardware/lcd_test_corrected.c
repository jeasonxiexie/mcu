...
int main(void)
{
    // CRITICAL: Lock power immediately to prevent shutdown!
    ...
    // Init UART debug (PA9 TX) and print boot banner
    Simple_UART_Init();
    Simple_UART_PrintLn("[BOOT] v2 starting");
    
    // Double-check power locking with HAL (PC6 = CON_POW_CPU, HIGH=开/保持)
    ...
    HAL_GPIO_WritePin(GPIOC, GPIO_PIN_6, GPIO_PIN_SET);  // HIGH = 开/保持
    Simple_UART_PrintLn("[PWR] PC6=HIGH (latched)");
    
    // Initialize LCD with corrected pin mappings
    Simple_UART_PrintLn("[LCD] init...");
    LCD_Init();
    Simple_UART_PrintLn("[LCD] init ok; backlight configured");
    ...
    while (1)
    {
        ...
        HAL_Delay(500);
        Simple_UART_PrintLn("[HB] running");
    }
}
...
// assert_failed is provided by stubs.c